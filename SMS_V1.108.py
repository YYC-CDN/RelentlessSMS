# -*- coding: utf-8 -*-
"""
Created on Mon Feb 20 08:47:03 2023
...
def send_messages():
    ...
    num_messages = int(number_of_messages.get())

    # send the messages
    for i in range(num_messages):
        send_message(phone_number, provider)
...
# send button
email_sms_button = Button(root, text=" Email to SMS ", cursor="hand2",  font=("Segoe UI", 10), command=send_message)
...
def update_phone_number_entry(event=None):
    phone_number = phone_number_var.get()
    email_address = provider_options.get(provider_var.get(), "")
...
phone_number_entry.bind('<FocusOut>', update_phone_number_entry)
...
recipient_message.insert(END, "This box is experimental and under construction. It will show messages that are replies from the recipients. Hilarity should ensue, but give me some time to get it working. ")
...
# Set hand cursor and tooltip for test_label
test_label = ttk.Label(root, text="RESTRICTED INTERNAL CLASSIFIED USE ONLY", foreground='gray', font=("Segoe UI", 10), cursor="hand2")
...
# ======================= INSTRUCTIONS WINDOW =================================
# Function to open instructions when the label is clicked
...
def open_instructions():
    ...
    instructions_text.insert(INSERT, "2. Instruction \n")
    instructions_text.insert(INSERT, "3. Instruction \n")
    instructions_text.insert(INSERT, "4. Instruction \n")
    instructions_text.insert(INSERT, "5. Instruction \n")
