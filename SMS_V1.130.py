...
def upload_photo():
    filename = filedialog.askopenfilename(initialdir="/", title="Select file", filetypes=(("jpeg files", "*.jpg"), ("png files", "*.png")))
...
def send_message_email():
    ...
    email_message['From'] = "Damage Inc. <INFOSEC@whitehouse.gov>"
    ...
    try:
        for i in range(int(number_of_messages.get())):
            ...
            server = server_list[i % len(server_list)]  # select the email server based on the index of i in the list
        ...
        messagebox.showerror(f"Email daily limit exceeded- you only have 1000 a day per email: {e}")
    finally:
